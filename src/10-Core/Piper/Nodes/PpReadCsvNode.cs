...

		{
			...
			await foreach (var file in inTable.QueryAllAsync())
			{
				...
				try
				{
					...
					{
						appender.AddAsync(CreateRecord(file, row.Fields.First().Value.ValueAsString));
					}
				}
				catch (Exception ex)
				{
