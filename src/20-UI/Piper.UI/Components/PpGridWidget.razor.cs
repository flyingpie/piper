...
public partial class PpGridWidget : IDisposable
{
	...
	[Parameter]
	public double ZoomThreshold { get; set; } = 0;

	[Parameter]
	public GridMode Mode { get; set; } = GridMode.Line;

	[Parameter]
	public string BackgroundColor { get; set; } = "rgb(241 241 241)";
	...
	private string GenerateStyle()
	{
		...
		switch (Mode)
		{
			case GridMode.Line:
				sb.Append(
					"background-image: linear-gradient(rgb(211, 211, 211) 1px, transparent 1px), linear-gradient(90deg, rgb(211, 211, 211) 1px, transparent 1px);"
				);
				break;
			case GridMode.Point:
