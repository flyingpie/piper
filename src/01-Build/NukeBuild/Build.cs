...
public sealed class Build : NukeBuild
{
	...
	private AbsolutePath PathToLinux64SelfContainedZip =>
		ArtifactsDirectory / $"linux-x64_self-contained.zip";
	...
	/// </summary>
	private Target PublishLinux64SelfContained =>
		_ =>
			_.DependsOn(Clean)
	...
	/// Windows x64 AOT.
	/// </summary>
	private Target PublishWin64Aot =>
		_ =>
			_.DependsOn(Clean)
	...
	/// </summary>
	private Target PublishWin64FrameworkDependent =>
		_ =>
			_.DependsOn(Clean)
	...
	/// Windows x64 self contained.
	/// </summary>
	private Target PublishWin64SelfContained =>
		_ =>
			_.DependsOn(Clean)
	...
	/// Scoop manifest.
	/// </summary>
	private Target CreateScoopManifest =>
		_ =>
			_.Executes(async () =>
			{
				...
				var sha256 = Convert
					.ToHexString(
						await SHA256.HashDataAsync(File.OpenRead(PathToWin64SelfContainedZip))
					)
					.ToLowerInvariant();

				var manifest = tpl.Replace(
						"$GH_RELEASE_VERSION$",
						$"v{SemVerVersion}",
				...
				await File.WriteAllTextAsync(
					RootDirectory / "scoop" / "wtq-nightly.json",
					manifest
				);
				await File.WriteAllTextAsync(
					RootDirectory / "scoop" / $"wtq-{SemVerVersion}.json",
					manifest
				);
			});
	...
	/// WinGet manifest.
	/// </summary>
	private Target CreateWinGetManifest =>
		_ =>
			_.Executes(async () =>
			{
	...
	/// GitHub release.
	/// </summary>
	private Target CreateGitHubRelease =>
		_ =>
			_.Description($"Creating release for the publishable version.")
				.Executes(async () =>
				{
					var token =
						GitHubActions?.Token
					...
					var ghRelease = await GitHubTasks.GitHubClient.GetOrCreateGitHubReleaseAsync(
						owner,
						name,
						SemVerVersion
					);
					...
					var latestChangeLog = await NukeExtensions.GetChangeLogEntryAsync(
						ChangeLogFile,
						SemVerVersion
					);
					...
					foreach (var ass in ghRelease.Assets)
					{
						...
						await GitHubTasks.GitHubClient.Repository.Release.DeleteAsset(
							owner,
							name,
							ass.Id
						);
					...
					// Upload new assets.
					await GitHubTasks.GitHubClient.UploadReleaseAssetToGithub(
						ghRelease,
						PathToLinux64FrameworkDependentZip
					);
					await GitHubTasks.GitHubClient.UploadReleaseAssetToGithub(
	...
	[SuppressMessage(
		...
		Justification = "MvdO: Invoked manually."
	)]
	private Target PublishRelease =>
		_ =>
			_.DependsOn(Clean)
				.DependsOn(PublishLinux64FrameworkDependent)
				.DependsOn(PublishLinux64SelfContained)
